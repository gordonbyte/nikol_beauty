 ...
 *
 * Only DiscountAutomaticBasic percentage discounts are supported for
 * writing; amount-off discounts are surfaced with supported:false so the
 * admin UI can explain why they can't be tracked yet.
 */
  ...
  do {
    ...
    for (const node of page.nodes) {
      ...
      discounts.push({
        ...
        supported: isPercentage, // amount-off: Phase 5
 ...
 * supported (percentage) discounts.
 *
 * Returns:
...
export async function computeTargetState(admin, activeDiscounts) {
  ...
  for (const { discount, pct, productIds } of perDiscount) {
    ...
    for (const v of variantTargets) applyPct(v.id, pct, discount.id);
    ...
    for (const productId of productIds) {
      ...
      if (!product) continue;
      for (const variantId of product.variantIds) applyPct(variantId, pct, discount.id);
    }
  }
